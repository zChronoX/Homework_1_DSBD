...
#Moduli gRPC generati dal file proto
import user_pb2
...
def delete_user():
    ...
    cur = conn.cursor()
    try:
        #Solo se email e password corrispondono, l'utente viene cancellato
        cur.execute("DELETE FROM users WHERE email=%s AND password=%s",
                    (email, hashlib.sha256(pwd.encode()).hexdigest()))
        conn.commit()
        if cur.rowcount > 0:
            #Se la cancellazione avviene con successo, aggiorno la cache, rimuovendo l'ID della richiesta
            #Mi permette di gestire il caso limite della cache
