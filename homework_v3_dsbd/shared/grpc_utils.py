...
def check_user_grpc(email):
    try:
        ...
        chan = grpc.insecure_channel(Config.GRPC_SERVER_ADDRESS)
        ...
        # Aggiungiamo una print per non diventare pazzi la prossima volta
        print(f"ERRORE gRPC CheckUser: {e}")
        return False


def verify_credentials_grpc(email, password):
    """
    Chiama lo User Manager via gRPC per verificare se email e password corrispondono.
    Restituisce True se valide, False altrimenti.
    """
    try:
        # Crea il canale verso lo User Manager (porta 50051)
        with grpc.insecure_channel(Config.GRPC_SERVER_ADDRESS) as channel:
            stub = user_pb2_grpc.UserManagerStub(channel)

            # Crea la richiesta con email e password
            request = user_pb2.CredentialsRequest(email=email, password=password)

            # Chiama il metodo remoto
            response = stub.VerifyUserCredentials(request)

            return response.exists
    except grpc.RpcError as e:
        print(f"Errore gRPC VerifyCredentials: {e}")
        return False
    except Exception as e:
        print(f"Errore generico gRPC: {e}")
        return False
