...
class Config:
    ...
    #Rendiamo le porte configurabili. Se K8s passa una variabile, la usiamo.
    #Altrimenti usiamo i vecchi default (5000 e 5001).

    FLASK_USER_PORT = int(os.getenv("FLASK_USER_PORT", 5000))
    FLASK_DATA_PORT = int(os.getenv("FLASK_DATA_PORT", 5001))


    #Indirizzo del Data Collector per chiamate HTTP interne (da User Manager)
    DATA_COLLECTOR_URL = os.getenv("DATA_COLLECTOR_URL", "http://data_collector:5001")




    #Endpoint per ottenere il token di autenticazione OAuth2 di OpenSky
    #Leggiamo le credenziali iniettate da Kubernetes Secrets.
    #I nomi "opensky-client-id" devono coincidere con le chiavi in secrets.yaml
    OPENSKY_TOKEN_URL = "https://auth.opensky-network.org/auth/realms/opensky-network/protocol/openid-connect/token"

    OPENSKY_CLIENT_ID = os.getenv("opensky-client-id")
    OPENSKY_CLIENT_SECRET = os.getenv("opensky-client-secret")
